...
{
public:
    StateMachine(sender *owner):m_owner(owner),m_currState(nullptr),m_previousState(nullptr),m_globalState(nullptr){};
    void setCurrState(State<sender>* s){ m_currState=s; };
    void setPreviousState(State<sender>* s){ m_previousState=s; };
    void setGlobalState(State<sender>* s){ m_globalState=s; };
    ...
    //改变状态
    void changeState(State<sender> *state)
    {
    ...
    bool isInState(const State<sender>* st) const
    {
        if(!m_currState)
        {
            return false;
...
private:
    sender *m_owner;
    State<sender> *m_currState;
    State<sender> *m_previousState;
    State<sender> *m_globalState;
};
